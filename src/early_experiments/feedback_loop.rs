	...
	fn on_event(&mut self, event: Event) -> SC2Result<()> {
		match event {
			...
			Event::UnitDestroyed(tag, alliance) => {
				let remove_mineral = |bot: &mut FeedbackLoop, tag| {
...
impl FeedbackLoop {
	fn build_overlord(&mut self) {
		// if an overlord is needed => ideally this has a better way of "leading"
		if self.supply_left < 3 && self.supply_cap < 200 {

			let lord = UnitTypeId::Overlord;
			// and one isn't currently being built => ideally we would build more than 1 at a time if we are negative
			if self.counter().ordered().count(lord) == 0 {

				// and we have larva + money (this part will be pulled out eventually)
				if !self.units.my.larvas.is_empty() && self.can_afford(lord, false) {
					if let Some(larva) = self.units.my.larvas.pop() {
						larva.train(lord, false);
						self.subtract_resources(lord, false);
					}
				}
			}
		}
